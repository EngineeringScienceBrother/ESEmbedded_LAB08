...
void init_usart1(void)
{
	...
	const double USARTDIV = SYSCLK_MHZ * 1.0e6 / 16 / BAUD;  // Because const and uint32_t double is ok

	const uint32_t DIV_MANTISSA = (uint32_t)USARTDIV;   
	const uint32_t DIV_FRACTION = (uint32_t)((USARTDIV-DIV_MANTISSA)*16);  // In fact, is 2.32
